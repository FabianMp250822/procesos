...
export default async function ProcessesPage({
  ...
  const { data: list, totalPages } = await getProcesses(q, currentPage, filters);
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-8 border-b border-slate-100">
        ...
        <div className="flex items-center gap-4">
          <ExportButton 
            ...
            action={exportProcesos.bind(null, q, filters)} 
          />
          <Link 
      ...
      <div className="bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-xl">
        ...
        <div className="p-8 border-b border-slate-100 flex flex-col md:flex-row gap-6 items-center justify-between bg-slate-50/50">
          <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
            <form className="relative w-full md:w-96" action="/dashboard/processes" method="GET">
              <Search className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
              <input 
                name="q"
                defaultValue={q}
                placeholder="Radicado, carpeta o demandante..."
                className="w-full pl-12 pr-5 py-4 bg-white border border-slate-200 rounded-2xl text-slate-900 focus:ring-2 focus:ring-[#C59D5F] outline-none transition-all placeholder:text-slate-300 font-bold text-sm"
              />
              {Object.entries(filters).map(([k, v]) => v && (
                <input key={k} type="hidden" name={k} value={v} />
              ))}
            </form>
            <ProcessFilters initialFilters={{ q, ...filters }} />
          </div>
